...
export  function setCountriesListeners(){
    ...
    countries.forEach(country => {
        country.addEventListener("click",()=>{
           ...
           console.log( displayCountry(country.dataset.countryName));
        })
    })
}
...
function displayCountry(countryName){
    Allcountries.then(countries=>{
         return {name,flags,translations,languages,maps,population,area,borders,capital,coatOfArms,continents,currencies,isoCode,idd:{root},landlocked,independent,tld,unMember , timezones , subregion , status  , startOfWeek , region , contentLoaded : false} =countries.filter(country => country.name.common  === countryName)[0] ?? {contentLoaded : false} ; 
    });
}
