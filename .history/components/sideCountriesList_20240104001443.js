...
function displayCountry(countryName){
    Allcountries.then(countries=>{
         let {name,flags,translations,area,borders,capital,coatOfArms,continents,currencies,isoCode,idd,landlocked,independent} =countries.filter(country => country.name.common  === countryName)[0] ; 
        console.log(idd.root)
    });
}
