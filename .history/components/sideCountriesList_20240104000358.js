...
function displayCountry(countryName){
    Allcountries.then(countries=>{
         let {name,flags,translations,area,borders,capital,coatOfArms,continents,currencies,isoCode,root} =countries.filter(country => country.name.common  === countryName)[0] ; 
        console.log(name)
    });
}
