...
function displayCountry(countryName){
    Allcountries.then(countries=>{
         let {name,flags,translations,languages,maps,population,area,borders,capital,coatOfArms,continents,currencies,isoCode,idd:{root},landlocked,independent} =countries.filter(country => country.name.common  === countryName)[0] ; 
        console.log(root)
    });
}
