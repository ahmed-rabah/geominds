...
export function countriesSearchList(list){
   return list.then((countries) => {
        ...
        countries.forEach(({name,flags}) => {
            htmlSearchList += `
                <div class="country-search-group closed-aside tooltip">
                    <span class="tooltip-body">${name.official}</span>
                    <img src="${flags.svg}" alt="${flags.alt}" class="country-icon-img" />
                    <h4 class="country-name" data-country-name="${flags.common}">${name.official}</h4>
                </div>
            `;
        })
