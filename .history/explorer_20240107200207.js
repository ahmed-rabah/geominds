...
function displayCountry(countryName){
    ...
    `; 
        ...
        let selectedTranslationItem = document.querySelector('.translation-item.selected') ; 
        console.log(selectedTranslationItem);
    ...
    for (let  key of nativeKeys) {
     ...
     translationsCode.forEach((code,index) => {
        (index == 0)  ? selected ="selected" : selected ="" ; 
         translationsList += `<li class="${selected} translation-item" data-language-code="${code}">${languageInfo(code)}</li>`;
     })
     translationsList += `</ul>`;
